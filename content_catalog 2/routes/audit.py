...
Blueprints:
- audit_bp: Prefixed with /admin/api/audit-logs when registered with the app
- audit_admin_bp: Prefixed with /admin/api when registered, provides GET /audit endpoint
...
# Create audit blueprint (legacy endpoint at /admin/api/audit-logs)
audit_bp = Blueprint('audit', __name__)

# Create admin audit blueprint (new endpoint at /admin/api/audit)
audit_admin_bp = Blueprint('audit_admin', __name__)
...
@audit_admin_bp.route('/audit', methods=['GET'])
@jwt_required()
def query_audit_logs():
    ...
    Returns a paginated list of audit logs with optional filtering by user,
    action type, resource type, date range, or IP address. This endpoint
    provides the same functionality as /admin/api/audit-logs but at the
    spec-required /admin/api/audit path.
