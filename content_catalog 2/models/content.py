...
class ContentApprovalRequest(db.Model):
    ...
    def __repr__(self):
        """String representation for debugging."""
        return f'<ContentApprovalRequest {self.id} - {self.status}>'

class MagicLinkToken(db.Model):
    """Token for magic link authentication (approval emails)."""
    __tablename__ = 'magic_link_tokens'
    
    id = db.Column(db.String(36), primary_key=True)
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=False)
    token = db.Column(db.String(255), unique=True, nullable=False)
    purpose = db.Column(db.String(50), nullable=False)  # 'approval_review', 'login', etc.
    asset_id = db.Column(db.String(36), db.ForeignKey('content_assets.id'), nullable=True)
