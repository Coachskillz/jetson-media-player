...
class User(db.Model):
    ...
    Attributes:
        ...
        tenant_ids: JSON array of tenant IDs the user belongs to
        can_approve_assets: Whether user can approve content assets
        can_fasttrack_unapproved_assets: Whether user can fast-track checkout unapproved assets
        fasttrack_expires_at: Timestamp when fast-track permission expires
        fasttrack_tenant_ids: JSON array of tenant IDs where fast-track applies
        allowed_catalog_ids: JSON array of catalog IDs user can access
        allowed_category_ids: JSON array of category IDs user can access
    """
    ...
    # Multi-tenant and asset workflow fields
    tenant_ids = db.Column(db.Text, nullable=True)  # JSON array of tenant IDs
    can_approve_assets = db.Column(db.Boolean, default=False, nullable=False)
    can_fasttrack_unapproved_assets = db.Column(db.Boolean, default=False, nullable=False)
    fasttrack_expires_at = db.Column(db.DateTime, nullable=True)
    fasttrack_tenant_ids = db.Column(db.Text, nullable=True)  # JSON array of tenant IDs for fast-track
    allowed_catalog_ids = db.Column(db.Text, nullable=True)  # JSON array of catalog IDs
    allowed_category_ids = db.Column(db.Text, nullable=True)  # JSON array of category IDs
    ...
    def to_dict(self):
        ...
        return {
            ...
            'can_approve_assets': self.can_approve_assets,
            'can_fasttrack_unapproved_assets': self.can_fasttrack_unapproved_assets,
            'fasttrack_expires_at': self.fasttrack_expires_at.isoformat() if self.fasttrack_expires_at else None,
            'fasttrack_tenant_ids': self.fasttrack_tenant_ids,
            'allowed_catalog_ids': self.allowed_catalog_ids,
            'allowed_category_ids': self.allowed_category_ids,
    ...
    def has_valid_fasttrack(self):
        """
        Check if the user has valid fast-track permission.

        Fast-track permission allows users to checkout unapproved assets.
        Permission is valid if:
        - can_fasttrack_unapproved_assets is True
        - fasttrack_expires_at is None (no expiry) or in the future

        Returns:
            True if user has valid fast-track permission, False otherwise
        """
        if not self.can_fasttrack_unapproved_assets:
            return False
        if self.fasttrack_expires_at is None:
            return True
        return datetime.now(timezone.utc) < self.fasttrack_expires_at

    def get_tenant_ids_list(self):
        """
        Get the list of tenant IDs the user belongs to.
    ...
    def set_tenant_ids_list(self, tenant_ids):
        """
        Set the list of tenant IDs the user belongs to.
    ...
    def get_fasttrack_tenant_ids_list(self):
        """
        Get the list of tenant IDs where fast-track applies.

        Returns:
            List of tenant IDs, or empty list if not set
        """
        import json
        if not self.fasttrack_tenant_ids:
            return []
        try:
            return json.loads(self.fasttrack_tenant_ids)
        except (json.JSONDecodeError, TypeError):
            return []

    def set_fasttrack_tenant_ids_list(self, tenant_ids):
        """
        Set the list of tenant IDs where fast-track applies.

        Args:
            tenant_ids: List of tenant IDs
        """
        import json
        self.fasttrack_tenant_ids = json.dumps(tenant_ids) if tenant_ids else None

    def get_allowed_catalog_ids_list(self):
        """
        Get the list of catalog IDs the user can access.

        Returns:
            List of catalog IDs, or empty list if not set
        """
        import json
        if not self.allowed_catalog_ids:
            return []
        try:
            return json.loads(self.allowed_catalog_ids)
        except (json.JSONDecodeError, TypeError):
            return []

    def set_allowed_catalog_ids_list(self, catalog_ids):
        """
        Set the list of catalog IDs the user can access.

        Args:
            catalog_ids: List of catalog IDs
        """
        import json
        self.allowed_catalog_ids = json.dumps(catalog_ids) if catalog_ids else None

    def get_allowed_category_ids_list(self):
        """
        Get the list of category IDs the user can access.

        Returns:
            List of category IDs, or empty list if not set
        """
        import json
        if not self.allowed_category_ids:
            return []
        try:
            return json.loads(self.allowed_category_ids)
        except (json.JSONDecodeError, TypeError):
            return []

    def set_allowed_category_ids_list(self, category_ids):
        """
        Set the list of category IDs the user can access.

        Args:
            category_ids: List of category IDs
        """
        import json
        self.allowed_category_ids = json.dumps(category_ids) if category_ids else None
