...
Called remotely by CMS via: POST http://{device_ip}:8080/api/command/{command}
"""
...
class SystemMonitor:
    ...
    def get_gpu_info() -> Dict[str, Any]:
        """Get NVIDIA GPU info (Jetson tegrastats)."""
        try:
    ...
    def get_temperature() -> Dict[str, float]:
        """Get system temperatures from thermal zones."""
        temps = {}
        try:
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div style="margin-top:15px">
                    ...
                    <button class="btn btn-danger" onclick="if(confirm('Reboot device?')) sendCommand('reboot')">Reboot</button>
                </div>
            </div>
        </div>
    ...
    <script>
        function updateDashboard() {
            fetch('/api/system').then(r => r.json()).then(data => {
                ...
                var tempsHtml = '';
                for (var k in data.temperature) {
                    tempsHtml += '<div class="stat"><span class="stat-label">' + k + '</span><span class="stat-value">' + data.temperature[k] + '°C</span></div>';
                }
                document.getElementById('temps').innerHTML = tempsHtml;
                var netHtml = '';
                for (var k in data.network) {
                    netHtml += '<div class="stat"><span class="stat-label">' + k + '</span><span class="stat-value">' + data.network[k].ip + '</span></div>';
                }
...
class HealthRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/command/minimize':
            ...
            threading.Thread(
                target=lambda: os.system('systemctl --user restart skillz-player'),
                daemon=True
            ).start()
            ...
            threading.Thread(
                target=lambda: os.system('sudo reboot'),
                daemon=True
            ).start()
    ...
    def _reset_pairing(self):
        ...
        if config_path.exists():
            try:
                ...
                logger.error("Failed to reset pairing: %s", e)


class HealthServer:
    """Health monitoring HTTP server running on port 8080."""
